...
class GetFileNameGUI(QMainWindow):
    ...
    def create_menu(self):
        ...
        file_menu_two = menu_bar.addMenu("&系统")

        exit_action = QAction("&退出", self)
        exit_action.triggered.connect(self.close_window)  # 退出窗口
        file_menu.addAction(exit_action)

        open_terminal_action = QAction("&打开终端", self)
        open_terminal_action.triggered.connect(self.open_terminal)
        file_menu_two.addAction(open_terminal_action)

        get_info_action = QAction("&获取系统信息", self)
        get_info_action.triggered.connect(self.get_system_info)
        file_menu_two.addAction(get_info_action)
    ...
    def open_terminal(self):
        if platform.system() == "Windows":
            os.system(f"start cmd /K cd {self.current_path}")
            self.output_text.append("成功打开终端")
        elif platform.system() == "Linux":
            os.system(f"x-terminal-emulator --working-directory={self.current_path}")
            self.output_text.append("成功打开终端")
